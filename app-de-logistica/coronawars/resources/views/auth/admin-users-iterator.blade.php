		...
		@foreach ($users as $user)
			<tr>
				<td>
					...
					<div class="dropdown">
					  ...
					  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
					  	...
					  	@if( $user->id != 1 )
						<a class="dropdown-item" href="{{ route(($user->isAdmin() ? 'remove-role' : 'assign-role'),['userid'=>$user->id,'rolename'=>'superadministrator']) }}">
							<span class="fa-stack " aria-hidden="true"><i class="fa {{ $user->isAdmin() ? 'fa-thumbs-down text-danger' : 'fa-thumbs-up'}} fa-stack-1x"></i><i class="fa fa-stack-2x fa-circle-o {{ $user->isAdmin() ? 'text-danger' : ''}} "></i></span>
							&nbsp;{{ !$user->isAdmin() ? __('Make admin') : __('Unmake admin')}}</a></li>
						@endif
						@endif
						<li><a class="dropdown-item" href="{{ route( ($user->hasRole('deliverer') ? 'remove-role':'assign-role'),['userid'=>$user->id,'rolename'=>'deliverer']) }}">
				...
				<td class="visible-md visible-lg">{{$user->created_at->diffForHumans()}}</td>
			</tr>
		@endforeach